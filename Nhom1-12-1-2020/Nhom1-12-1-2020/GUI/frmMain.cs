...
namespace Nhom1_12_1_2020
{
    public partial class FormMain : Form
    {
         ...
         void LoadDanhSachLopHoc()
        {
            ...
            // Nên sử dụng cách này load nhanh hơn
            var ls = db.Classrooms.Select(e => new ClassRoomViewModel { 
                ID = e.ID,
                Name = e.Name,
                Room = e.Room,
                TotalStudent = e.Students.Count,
                TotalMale = e.Students.Where( a => a.Gender == 1).Count(),
                TotalFemale = e.Students.Where(a => a.Gender == 0).Count()
        ...
        {
            var lopDangChon = bdLopHoc.Current as ClassRoomViewModel;
            if(lopDangChon != null)
            {
        ...
        private void bdLopHoc_CurrentChanged(object sender, EventArgs e)
        {
            ...
            if(lopDangChon != null)
            {
                ...
                var dsSV = db.Students.
                    Where(t => t.IDClassroom == lopDangChon.ID).ToList(); // Lớp vô danh
